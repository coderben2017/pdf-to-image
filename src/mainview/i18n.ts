const messages = {
  zh: {
    brand: "PDF 转图片",
    localOnly: "本地处理",
    starProject: "Star",
    starProjectAria: "在 GitHub 为项目加星",
    eyebrow: "高清 · 无损 · 不上传",
    title: "PDF 转图片并添加文字",
    subtitle: "专为表格和小字号文档优化。点击预览定位文字，再导出 300 DPI 无损 PNG。",
    dropTitle: "拖入 PDF，或点击选择",
    dropHint: "推荐单页表格文件，最大 100 MB",
    loadingPdf: "正在读取 PDF…",
    loadingPdfHint: "文件仍只在当前设备处理",
    privacy: "文件不离开设备",
    tableReady: "细线清晰保留",
    highDpi: "最高 600 DPI",
    replaceFile: "更换文件",
    settingsKicker: "导出设置",
    settingsTitle: "文字与清晰度",
    reset: "重置",
    textContent: "贴字内容",
    textPlaceholder: "审核通过",
    textHint: "支持多行，按 Enter 换行",
    resolution: "输出精度",
    format: "图片格式",
    opacity: "透明度",
    angle: "文字角度",
    textColor: "文字颜色",
    positionX: "横向位置",
    positionY: "纵向位置",
    fontSize: "文字字号",
    preparing: "正在准备…",
    exportPng: "导出高清 PNG",
    exportJpeg: "导出高清 JPEG",
    exportZip: "导出全部页面 ZIP",
    exportHint: "A4 约 2480 × 3508 px",
    previewKicker: "实时预览",
    previewTitle: "第 1 页",
    previewReady: "预览已更新",
    zoomLabel: "预览缩放",
    zoomOut: "缩小预览",
    zoomIn: "放大预览",
    rendering: "正在渲染预览…",
    previewNote: "拖动文字定位；滚轮缩放，按住 PDF 可平移预览。",
    footer: "浏览器本地处理 · 不留存任何文件",
    filePages: "{pages} 页 · {size}",
    exportProgress: "正在处理第 {current}/{total} 页",
    complete: "贴字图片已生成",
    invalidFile: "请选择有效的 PDF 文件",
    fileTooLarge: "PDF 文件不能超过 100 MB",
    loadFailed: "PDF 读取失败，请确认文件未损坏或加密",
    exportFailed: "导出失败，请降低 DPI 后重试",
  },
  en: {
    brand: "PDF to Image",
    localOnly: "On-device",
    starProject: "Star",
    starProjectAria: "Star this project on GitHub",
    eyebrow: "Sharp · Lossless · Private",
    title: "Turn PDFs into images with text",
    subtitle: "Built for tables and small text. Click to position text, then export a lossless 300 DPI PNG.",
    dropTitle: "Drop a PDF, or choose one",
    dropHint: "Best for one-page tables, up to 100 MB",
    loadingPdf: "Reading PDF…",
    loadingPdfHint: "The file stays on this device",
    privacy: "Stays on this device",
    tableReady: "Crisp lines preserved",
    highDpi: "Up to 600 DPI",
    replaceFile: "Replace file",
    settingsKicker: "EXPORT SETUP",
    settingsTitle: "Text & quality",
    reset: "Reset",
    textContent: "Text to place",
    textPlaceholder: "APPROVED",
    textHint: "Multiple lines; Enter to break",
    resolution: "Resolution",
    format: "Image format",
    opacity: "Opacity",
    angle: "Text angle",
    textColor: "Text color",
    positionX: "Horizontal",
    positionY: "Vertical",
    fontSize: "Text size",
    preparing: "Preparing…",
    exportPng: "Export HD PNG",
    exportJpeg: "Export HD JPEG",
    exportZip: "Export all as ZIP",
    exportHint: "A4 is about 2480 × 3508 px",
    previewKicker: "LIVE PREVIEW",
    previewTitle: "Page 1",
    previewReady: "Preview updated",
    zoomLabel: "Preview zoom",
    zoomOut: "Zoom out",
    zoomIn: "Zoom in",
    rendering: "Rendering preview…",
    previewNote: "Drag text to place it; scroll to zoom, or hold and drag the PDF to pan.",
    footer: "Processed on-device · No files retained",
    filePages: "{pages} pages · {size}",
    exportProgress: "Processing page {current}/{total}",
    complete: "Placed-text image is ready",
    invalidFile: "Please choose a valid PDF file",
    fileTooLarge: "PDF files must be under 100 MB",
    loadFailed: "Could not read this PDF. It may be damaged or encrypted",
    exportFailed: "Export failed. Try a lower DPI",
  },
} as const;

export type Locale = keyof typeof messages;
export type MessageKey = keyof typeof messages.zh;

let locale: Locale = navigator.language.toLowerCase().startsWith("zh") ? "zh" : "en";

export const getLocale = () => locale;

export const setLocale = (nextLocale: Locale) => {
  locale = nextLocale;
};

export const t = (key: MessageKey, params: Record<string, string | number> = {}) => {
  let result: string = messages[locale][key];
  Object.entries(params).forEach(([name, value]) => {
    result = result.replace(`{${name}}`, String(value));
  });
  return result;
};

export const applyTranslations = () => {
  document.documentElement.lang = locale === "zh" ? "zh-CN" : "en";
  document.querySelectorAll<HTMLElement>("[data-i18n]").forEach((element) => {
    const key = element.dataset.i18n as MessageKey;
    element.textContent = t(key);
  });
  document.querySelectorAll<HTMLElement>("[data-i18n-placeholder]").forEach((element) => {
    const key = element.dataset.i18nPlaceholder as MessageKey;
    element.setAttribute("placeholder", t(key));
  });
  document.querySelectorAll<HTMLElement>("[data-i18n-aria]").forEach((element) => {
    const key = element.dataset.i18nAria as MessageKey;
    element.setAttribute("aria-label", t(key));
  });
};
